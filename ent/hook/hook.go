...
// The InvestorFunc type is an adapter to allow the use of ordinary
// function as Investor mutator.
type InvestorFunc func(context.Context, *ent.InvestorMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f InvestorFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.InvestorMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.InvestorMutation", m)
}
