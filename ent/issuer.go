...
func (*Issuer) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case issuer.ForeignKeys[0]: // issuer_balance
...
func (i *Issuer) assignValues(columns []string, values []any) error {
	...
	for j := range columns {
		...
		case issuer.ForeignKeys[0]:
			if value, ok := values[j].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field issuer_balance", values[j])
			} else if value.Valid {
				i.issuer_balance = new(uuid.UUID)
				*i.issuer_balance = *value.S.(*uuid.UUID)
			}
		default:
...
func (i *Issuer) Update() *IssuerUpdateOne {
	return NewIssuerClient(i.config).UpdateOne(i)
}
